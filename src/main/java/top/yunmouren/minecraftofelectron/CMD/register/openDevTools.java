...
public class openDevTools  implements Command<CommandSource> {
    public static openDevTools instance = new openDevTools();
    @Override
    public int run(CommandContext<CommandSource> context) throws CommandSyntaxException {
        Entity onlinePlayerNames = context.getSource().getEntity();
        if (onlinePlayerNames instanceof PlayerEntity) {
            serverSendPack(onlinePlayerNames.getDisplayName().getString(), "openDevTools ");
        }
        return 0;
    }
}
